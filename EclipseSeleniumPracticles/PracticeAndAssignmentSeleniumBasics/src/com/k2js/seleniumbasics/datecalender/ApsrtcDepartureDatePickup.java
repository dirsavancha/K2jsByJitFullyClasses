...
public class ApsrtcDepartureDatePickup {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		System.out.println("All row size"+fromdate_row_size);
		
		List<WebElement> each_ele=driver.findElements(By.xpath("//div[@class='ui-datepicker-group ui-datepicker-group-first']/table/tbody/tr[1]/td"));
		int each_ele_col=each_ele.size();
		System.out.println("Size of total elements "+each_ele_col);
		
		WebElement el; 
		for(int i=1;i<=fromdate_row_size;i++) {
			for(int j=1;j<=each_ele_col;j++) {
				 el=driver.findElement(By.xpath("//div[@class='ui-datepicker-group ui-datepicker-group-first']/table/tbody/tr["+i+"]/td["+j+"]"));
